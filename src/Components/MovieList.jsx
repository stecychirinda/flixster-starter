  ...
  const handleModal = async (movie_id) => {
    try {
      const response = await fetch(
        `https://api.themoviedb.org/3/movie/${movie_id}?api_key=${apiSecret}`,
      );
      const dataSet = await response.json();

      const trailerResult = await fetch(
        `https://api.themoviedb.org/3/movie/${movie_id}/videos?api_key=${apiSecret}`,
      );
